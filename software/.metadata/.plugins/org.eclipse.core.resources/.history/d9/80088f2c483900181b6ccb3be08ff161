...
#define SEVEN_SEG_PRIORITY		    8

#define LCD_TASK2_PRIORITY			7
#define LCD_TASK1_PRIORITY			6
...
// counter_task is a task for incrementing a counter every second and displayin on seven seg
void counter_task(void *pvParameters) {
	int counter = 0;
	while (1)
	{
		counter++;
		// write a value to seven seg
...
void lcd_task1(void *pvParameters) {
	while (1)
	{
		...
		fprintf((FILE*)pvParameters, "%c%s", ESC, CLEAR_LCD_STRING);
		fprintf((FILE*)pvParameters, "LCD Task 1\n");
...
int initOSDataStructs(void)
{
	...
	lcd_sem = xSemaphoreCreateCounting(9999, 1);
	return 0;
}
...
int initCreateTasks(FILE* lcd)
{
	xTaskCreate(lcd_task2, "lcd_task2", TASK_STACKSIZE, lcd, LCD_TASK2_PRIORITY, NULL);
	xTaskCreate(lcd_task1, "lcd_task1", TASK_STACKSIZE, lcd, LCD_TASK1_PRIORITY, NULL);
	xTaskCreate(counter_task, "counter_task", TASK_STACKSIZE, NULL, SEVEN_SEG_PRIORITY, NULL);
